...
describe NytBesties::BestSellers do
  ...
  context "when it results from a list endpoint" do
    before :each do
    ...
    it "should not have a history" do
      @best.has_history?.should be_false
    end
  end

  context "when is results from a history endpoint" do
    before :each do
      data = JSON.parse(HISTORY_RESPONSE)['results'].first
      @best = NytBesties::BestSellers.new(data, @client)
    ...
    it "should have a history" do
      @best.has_history?.should be_true
    end

    describe "#history" do
      it "should be an array" do
        @best.history.should be_an_instance_of Array
      end

      it "should be an Array of Rank objects" do
        @best.history.first.should be_an_instance_of NytBesties::Rank
      end
    end
  end
end
